...
function App() {
    ...
    const wrapText = (ctx, text, x, y, maxWidth, lineHeight) => {
     ...
     const loadRandomImage = () => {
        const randomIndex = Math.floor(Math.random() * 10) + 1; // Genera un número aleatorio entre 1 y 10
        return new Promise((resolve, reject) => {
            const img = new Image();
            img.onload = () => resolve(img);
            img.onerror = reject;
            img.src = `/web/${randomIndex}.png`;
        });
    };
    ...
    const drawCsvDataOnCanvas = async (data, index) => {
      ...
      const image = await loadRandomImage();
        ...
        // Determinar una posición aleatoria para la imagen
        let imgX, imgY;
        ...
        switch (position) {
            case 'top':
                imgX = Math.random() * (canvas.width - image.width);
                imgY = margin;
                break;
            case 'bottom':
                imgX = Math.random() * (canvas.width - image.width);
                imgY = canvas.height - image.height - margin;
                break;
            case 'left':
                imgX = margin;
                imgY = Math.random() * (canvas.height - image.height);
                break;
            case 'right':
                imgX = canvas.width - image.width - margin;
                imgY = Math.random() * (canvas.height - image.height);
                break;
        }
        ...
        if (item.caracteristicas) {
            ...
            for (let i = 0; i < characteristics.length; i++) {
              const text = `• ${characteristics[i].trim()}`; 
        ...
        if (item.boton) {
            ...
            const extraSpaceAbove = -55; // Espacio extra por encima del botón, ajustar según necesidad
            const buttonY = currentY - buttonHeight / 2 - extraSpaceAbove;
            const textY = buttonY + buttonPaddingVertical + textMetrics.actualBoundingBoxAscent + 10;
            ...
            currentY += buttonHeight +extraSpaceAbove + 20; // Añadir margen después del botón
        }
